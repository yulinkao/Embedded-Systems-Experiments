...
#define  FASTFLASHTIME    50	  // 短延时（50ms-100ms）
#define  SLOWFLASHTIME    3000     // 长延时（500ms-1s）
...
void GPIOInit(void)
{
    ...
    // 设置端口 F的第1位（PF1）为输出引脚
    GPIOPinTypeGPIOOutput(GPIO_PORTF_BASE, GPIO_PIN_1);			

    // 设置端口 N的第0位（PN0）为输出引脚			
		//GPIOPinTypeGPIOOutput(GPIO_PORTN_BASE, GPIO_PIN_0);
			
    // 设置端口 N的第1位（PN1）为输出引脚			
...
//{
	  //if (ui8KeyValue	== 0)                                  // PUSH1(USR_SW1-PJ0) 按下
			//GPIOPinWrite(GPIO_PORTN_BASE, GPIO_PIN_3, GPIO_PIN_3); // 点亮 LED4(D4-PF0)
    //else                                                   // PUSH1(USR_SW1-PJ0) 松开
			//GPIOPinWrite(GPIO_PORTF_BASE, GPIO_PIN_3, 0x0);        // 关闭 LED4(D4-PF0)
//}
