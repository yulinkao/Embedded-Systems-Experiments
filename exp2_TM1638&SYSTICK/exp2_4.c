		...
		if (clock100ms_flag == 1)      // 检查0.1秒定时是否到
		{
			...
			if (flag ==0)
			{
				...
				{	
					...
					digit[5]='A';
					digit[6]='A';
					if (--test_counter <= 0) test_counter = 10000;
				...
				}else
				{
					...
					digit[0] = test_counter / 1000; 	    // ?????
					digit[1] = test_counter / 100 % 10; 	// ?????
					digit[2] = test_counter / 10 % 10; 	  // ?????
					digit[3] = test_counter % 10;         // ??????
				}
			}else
			{
				led[6] = 1;
